...
|  ( 1, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.EQNLIST 
EQNLIST1, _, _)) :: ( _, ( _, LPAREN1left, _)) :: rest671)) => let
 val  result = MlyValue.PROG (fn _ => let val  (EQNLIST as EQNLIST1) =
 EQNLIST1 ()
 in (EQNLIST)
end)
 in ( LrTable.NT 1, ( result, LPAREN1left, RPAREN1right), rest671)
end
|  ( 2, ( ( _, ( _, _, RPAREN2right)) :: ( _, ( MlyValue.EXP EXP1, _,
 _)) :: _ :: ( _, ( MlyValue.ID ID1, _, _)) :: _ :: _ :: ( _, ( _, 
LPAREN1left, _)) :: rest671)) => let val  result = MlyValue.EQN (fn _
 => let val  (ID as ID1) = ID1 ()
 val  (EXP as EXP1) = EXP1 ()
 in (ID, (nil,EXP))
end)
 in ( LrTable.NT 2, ( result, LPAREN1left, RPAREN2right), rest671)
end
|  ( 3, ( ( _, ( _, _, RPAREN2right)) :: ( _, ( MlyValue.EXP EXP1, _,
 _)) :: _ :: ( _, ( MlyValue.VARLIST VARLIST1, _, _)) :: ( _, ( 
MlyValue.ID ID1, _, _)) :: _ :: _ :: ( _, ( _, LPAREN1left, _)) :: 
rest671)) => let val  result = MlyValue.EQN (fn _ => let val  (ID as 
ID1) = ID1 ()
...
|  ( 4, ( ( _, ( MlyValue.EQN EQN1, EQN1left, EQN1right)) :: rest671))
 => let val  result = MlyValue.EQNLIST (fn _ => let val  (EQN as EQN1)
 = EQN1 ()
 in (let val (fname,body) = EQN in (fname::nil,body::nil) end)
end)
 in ( LrTable.NT 3, ( result, EQN1left, EQN1right), rest671)
end
|  ( 5, ( ( _, ( MlyValue.EQNLIST EQNLIST1, _, EQNLIST1right)) :: ( _,
 ( MlyValue.EQN EQN1, EQN1left, _)) :: rest671)) => let val  result = 
MlyValue.EQNLIST (fn _ => let val  (EQN as EQN1) = EQN1 ()
 val  (EQNLIST as EQNLIST1) = EQNLIST1 ()
 in (
let val (flist,blist) = EQNLIST; val (fname,body) = EQN in (fname::flist,body::blist) end
)
end)
 in ( LrTable.NT 3, ( result, EQN1left, EQNLIST1right), rest671)
end
|  ( 6, ( ( _, ( MlyValue.CONST CONST1, CONST1left, CONST1right)) :: 
rest671)) => let val  result = MlyValue.EXP (fn _ => let val  (CONST
 as CONST1) = CONST1 ()
 in (SchemeAST.Const(CONST))
end)
...
|  ( 7, ( ( _, ( MlyValue.ID ID1, ID1left, ID1right)) :: rest671)) =>
 ...
 in (SchemeAST.Variable(ID))
end)
 in ( LrTable.NT 4, ( result, ID1left, ID1right), rest671)
end
|  ( 8, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.EXP EXP3, _,
 _)) :: ( _, ( MlyValue.EXP EXP2, _, _)) :: ( _, ( MlyValue.EXP EXP1,
 _, _)) :: _ :: ( _, ( _, LPAREN1left, _)) :: rest671)) => let val  
result = MlyValue.EXP (fn _ => let val  EXP1 = EXP1 ()
 val  EXP2 = EXP2 ()
 val  EXP3 = EXP3 ()
 in (SchemeAST.If(EXP1,EXP2,EXP3))
end)
 in ( LrTable.NT 4, ( result, LPAREN1left, RPAREN1right), rest671)
end
|  ( 9, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.EXPLIST 
EXPLIST1, _, _)) :: ( _, ( MlyValue.ID ID1, _, _)) :: _ :: ( _, ( _, 
LPAREN1left, _)) :: rest671)) => let val  result = MlyValue.EXP (fn _
 => let val  (ID as ID1) = ID1 ()
 val  (EXPLIST as EXPLIST1) = EXPLIST1 ()
 in (SchemeAST.Call(ID,EXPLIST))
end)
 in ( LrTable.NT 4, ( result, LPAREN1left, RPAREN1right), rest671)
end
|  ( 10, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.EXPLIST 
EXPLIST1, _, _)) :: ( _, ( MlyValue.OP OP1, _, _)) :: ( _, ( _, 
LPAREN1left, _)) :: rest671)) => let val  result = MlyValue.EXP (fn _
 => let val  (OP as OP1) = OP1 ()
 val  (EXPLIST as EXPLIST1) = EXPLIST1 ()
 in (SchemeAST.Op(OP,EXPLIST))
end)
 in ( LrTable.NT 4, ( result, LPAREN1left, RPAREN1right), rest671)
end
|  ( 11, ( ( _, ( MlyValue.EXP EXP1, EXP1left, EXP1right)) :: rest671)
...
|  ( 19, ( ( _, ( MlyValue.ID ID1, ID1left, ID1right)) :: rest671)) =>
 let val  result = MlyValue.QUOTABLE (fn _ => let val  (ID as ID1) = 
ID1 ()
 in (SchAST.Symbol ID)
end)
 in ( LrTable.NT 7, ( result, ID1left, ID1right), rest671)
end
|  ( 20, ( ( _, ( MlyValue.OP OP1, OP1left, OP1right)) :: rest671)) =>
 let val  result = MlyValue.QUOTABLE (fn _ => let val  (OP as OP1) = 
OP1 ()
 in (SchAST.Symbol OP)
end)
 in ( LrTable.NT 7, ( result, OP1left, OP1right), rest671)
end
|  ( 21, ( ( _, ( _, IF1left, IF1right)) :: rest671)) => let val  
result = MlyValue.QUOTABLE (fn _ => (SchAST.Symbol "if"))
 in ( LrTable.NT 7, ( result, IF1left, IF1right), rest671)
end
|  ( 22, ( ( _, ( _, CALL1left, CALL1right)) :: rest671)) => let val  
result = MlyValue.QUOTABLE (fn _ => (SchAST.Symbol "call"))
 in ( LrTable.NT 7, ( result, CALL1left, CALL1right), rest671)
end
|  ( 23, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.QUOTABLELIST
 QUOTABLELIST1, _, _)) :: ( _, ( _, LPAREN1left, _)) :: rest671)) =>
 let val  result = MlyValue.QUOTABLE (fn _ => let val  (QUOTABLELIST
 as QUOTABLELIST1) = QUOTABLELIST1 ()
 in (List.foldr SchemeAST.List SchemeAST.NIL QUOTABLELIST)
end)
...
|  ( 25, ( ( _, ( MlyValue.VARLIST VARLIST1, _, VARLIST1right)) :: ( _
, ( MlyValue.ID ID1, ID1left, _)) :: rest671)) => let val  result = 
MlyValue.VARLIST (fn _ => let val  (ID as ID1) = ID1 ()
 val  (VARLIST as VARLIST1) = VARLIST1 ()
 in (ID::VARLIST)
end)
 in ( LrTable.NT 10, ( result, ID1left, VARLIST1right), rest671)
end
|  ( 26, ( ( _, ( MlyValue.QUOTABLE QUOTABLE1, QUOTABLE1left, 
QUOTABLE1right)) :: rest671)) => let val  result = 
MlyValue.QUOTABLELIST (fn _ => let val  (QUOTABLE as QUOTABLE1) = 
QUOTABLE1 ()
 in (QUOTABLE::nil)
end)
 in ( LrTable.NT 8, ( result, QUOTABLE1left, QUOTABLE1right), rest671)

end
|  ( 27, ( ( _, ( MlyValue.QUOTABLELIST QUOTABLELIST1, _, 
QUOTABLELIST1right)) :: ( _, ( MlyValue.QUOTABLE QUOTABLE1, 
QUOTABLE1left, _)) :: rest671)) => let val  result = 
MlyValue.QUOTABLELIST (fn _ => let val  (QUOTABLE as QUOTABLE1) = 
QUOTABLE1 ()
 val  (QUOTABLELIST as QUOTABLELIST1) = QUOTABLELIST1 ()
 in (QUOTABLE::QUOTABLELIST)
end)
 in ( LrTable.NT 8, ( result, QUOTABLE1left, QUOTABLELIST1right), 
rest671)
end
|  ( 28, ( ( _, ( MlyValue.PLUS PLUS1, PLUS1left, PLUS1right)) :: 
rest671)) => let val  result = MlyValue.OP (fn _ => let val  (PLUS as 
PLUS1) = PLUS1 ()
 in (PLUS)
end)
 in ( LrTable.NT 9, ( result, PLUS1left, PLUS1right), rest671)
end
|  ( 29, ( ( _, ( MlyValue.MINUS MINUS1, MINUS1left, MINUS1right)) :: 
rest671)) => let val  result = MlyValue.OP (fn _ => let val  (MINUS
 as MINUS1) = MINUS1 ()
 in (MINUS)
end)
 in ( LrTable.NT 9, ( result, MINUS1left, MINUS1right), rest671)
end
|  ( 30, ( ( _, ( MlyValue.EQL EQL1, EQL1left, EQL1right)) :: rest671)
) => let val  result = MlyValue.OP (fn _ => let val  (EQL as EQL1) = 
EQL1 ()
 in (EQL)
end)
 in ( LrTable.NT 9, ( result, EQL1left, EQL1right), rest671)
end
|  ( 31, ( ( _, ( MlyValue.TIMES TIMES1, TIMES1left, TIMES1right)) :: 
rest671)) => let val  result = MlyValue.OP (fn _ => let val  (TIMES
 as TIMES1) = TIMES1 ()
 in (TIMES)
end)
 in ( LrTable.NT 9, ( result, TIMES1left, TIMES1right), rest671)
end
|  ( 32, ( ( _, ( MlyValue.DIV DIV1, DIV1left, DIV1right)) :: rest671)
) => let val  result = MlyValue.OP (fn _ => let val  (DIV as DIV1) = 
DIV1 ()
 in (DIV)
end)
 in ( LrTable.NT 9, ( result, DIV1left, DIV1right), rest671)
end
|  ( 33, ( ( _, ( MlyValue.MOD MOD1, MOD1left, MOD1right)) :: rest671)
) => let val  result = MlyValue.OP (fn _ => let val  (MOD as MOD1) = 
MOD1 ()
 in (MOD)
end)
 in ( LrTable.NT 9, ( result, MOD1left, MOD1right), rest671)
end
|  ( 34, ( ( _, ( MlyValue.CONS CONS1, CONS1left, CONS1right)) :: 
rest671)) => let val  result = MlyValue.OP (fn _ => let val  (CONS as 
CONS1) = CONS1 ()
 in (CONS)
end)
 in ( LrTable.NT 9, ( result, CONS1left, CONS1right), rest671)
end
|  ( 35, ( ( _, ( MlyValue.CAR CAR1, CAR1left, CAR1right)) :: rest671)
) => let val  result = MlyValue.OP (fn _ => let val  (CAR as CAR1) = 
CAR1 ()
 in (CAR)
end)
 in ( LrTable.NT 9, ( result, CAR1left, CAR1right), rest671)
end
|  ( 36, ( ( _, ( MlyValue.CDR CDR1, CDR1left, CDR1right)) :: rest671)
) => let val  result = MlyValue.OP (fn _ => let val  (CDR as CDR1) = 
CDR1 ()
 in (CDR)
end)
 in ( LrTable.NT 9, ( result, CDR1left, CDR1right), rest671)
end
|  ( 37, ( ( _, ( MlyValue.NULL NULL1, NULL1left, NULL1right)) :: 
rest671)) => let val  result = MlyValue.OP (fn _ => let val  (NULL as 
NULL1) = NULL1 ()
 in (NULL)
end)
 in ( LrTable.NT 9, ( result, NULL1left, NULL1right), rest671)
end
