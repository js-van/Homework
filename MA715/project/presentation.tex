...
\end{columns}
\pause
Need to compute $\frac{\partial C_{i,j}(R, y)}{\partial y}$, $\frac{\partial C_{i,j}(R, y)}{\partial R}$
\vskip5pt
...
\end{eqnarray*}
\pause
Unfortunately not a Fourier multiplier.
\vskip10pt
\pause
Can be precalculated with $O(d)$ overhead.
\end{frame}
